...
class CheckSeasonEndings extends Command
{
     ...
     */
    public function handle(SeasonService $seasonService, MessageService $messageService, MessageTrackingService $trackingService): int
    {
        ...
        foreach ($groupsToEnd as $group) {
            ...
            // Track the message attempt if season was successfully ended
            if ($endedSeason) {
                try {
                    $trackingService->recordMessage(
                        $group,
                        'season.ended',
                        "Season {$endedSeason->season_number} ended automatically",
                        'season',
                        $endedSeason->id,
                        [
                            'season_number' => $endedSeason->season_number,
