...
class SendSeasonMilestoneDrops implements ShouldQueue
{
    ...
    public function handle(MessageTrackingService $trackingService): void
    {
        ...
        foreach ($groups as $group) {
            $this->processGroupMilestones($group, $trackingService);
        }
    }
     ...
     */
    private function processGroupMilestones(Group $group, MessageTrackingService $trackingService): void
    {
        ...
        foreach (self::MILESTONES as $threshold => $amount) {
            ...
            if ($seasonProgress >= $threshold) {
                $this->triggerMilestoneDrop($group, $threshold, $amount, $trackingService);
     ...
     */
    private function triggerMilestoneDrop(Group $group, int $threshold, int $amount, MessageTrackingService $trackingService): void
    {
        $recipientsCount = 0;
        ...
        $trackingService->recordMessage(
            $group,
            'season.milestone',
            "Season {$threshold}% milestone: {$amount} points distributed",
            'season',
