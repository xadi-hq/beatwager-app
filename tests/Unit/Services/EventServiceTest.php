...
class EventServiceTest extends TestCase
{
    ...
    public function test_attendance_bonus_updates_last_wager_joined_for_decay(): void
    {
        ...
        $this->group->users()->attach($user2->id, [
            'id' => Str::uuid(),
            'points' => 1000,
            'role' => 'participant',
        ]);

        // Mock point service - expect 2 calls (one for each attendee)
        $this->pointService->expects($this->exactly(2))
            ->method('awardPoints')
            ->with(
                $this->callback(function ($arg) use ($user1, $user2) {
                    return $arg->id === $user1->id || $arg->id === $user2->id;
                }),
                ...
                $this->callback(function ($arg) use ($event) {
                    return $arg->id === $event->id;
                })
            );

        // Record attendance for both users (minimum 2 required)
        $this->eventService->recordAttendance($event, $this->creator, [$user1->id, $user2->id]);

        // Verify last_wager_joined_at was updated for first user
        $userGroup = $this->group->users()->where('user_id', $user1->id)->first();
        $this->assertNotNull($userGroup->pivot->last_wager_joined_at);
        $this->assertTrue(
            Carbon::parse($userGroup->pivot->last_wager_joined_at)->isToday()
        );

        // Verify bonus was marked as awarded for both users
        $attendance1 = GroupEventAttendance::where('event_id', $event->id)
            ->where('user_id', $user1->id)
            ->first();
        $this->assertTrue($attendance1->bonus_awarded);

        $attendance2 = GroupEventAttendance::where('event_id', $event->id)
            ->where('user_id', $user2->id)
            ->first();
        $this->assertTrue($attendance2->bonus_awarded);
    }
}